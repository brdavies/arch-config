Kotlin.defineModule('browser-ext-bootstrap', ['jb-chrome-ext-api', 'chrome-ext-options', 'browser-ext-platform'], function () {
  ...
  Kotlin.p(_, 'com_jetbrains_browserConnection_bootstrap', function () {
    ...
    function $f2(it) {
      if (!Kotlin.equals(it.host, _.com_jetbrains_browserConnection_bootstrap.lastHost) || !Kotlin.equals(it.port, _.com_jetbrains_browserConnection_bootstrap.lastPort)) {
        ...
        _.com_jetbrains_browserConnection_bootstrap.connect(Kotlin.equals(it.host, "localhost") ? "127.0.0.1" : it.host, it.port, connectDisposable);
    ...
    function $f9(buildInfo, host, port, version, ideLogName, currentConnectedDisposable, it) {
      var tmp$0;
      ((tmp$0 = it.contentWindow) !== null && tmp$0 !== undefined ? tmp$0 : Kotlin.throwNPE()).connect(buildInfo, host, port, $f8.bind(null, version, ideLogName, currentConnectedDisposable), currentConnectedDisposable);
    }
    function $fa(request, reconnectTimer, send$ref, disposable, attemptsCount, url, tryIdea120x, host, port) {
      ...
      if (request.status === 200) {
        ...
        var buildInfo = tryIdea120x.v ? Kotlin.modules['browser-ext-platform'].com_jetbrains_browserConnection.BuildInfo("123.0", 0, -1) : JSON.parse(responseText);
        var ideLogName = buildInfo.name + " (" + host + ":" + port + ")";
        var version = "latest";
        _.com_jetbrains_browserConnection_bootstrap.getFrame(version, ideLogName, $f9.bind(null, buildInfo, host, port, version, ideLogName, currentConnectedDisposable));
      }
    ...
    Object.defineProperty(this, 'connect', {value: function (host, port, disposable) {
      ...
      var send$ref, send = $fa.bind(this, request, reconnectTimer, send$ref = {r: null}, disposable, attemptsCount, url, tryIdea120x, host, port);
      send$ref.r = send;
      send();
    }});
    Object.defineProperty(this, 'getFrame', {value: function (version, ideLogName, callback) {
      ...
      if (frame === null || frame === undefined) {
        _.com_jetbrains_browserConnection_bootstrap.loadFrame(systemId, version, ideLogName, callback);
      }
       else {
    ...
    function $fb(version, ideLogName, callback, frame) {
    ...
    Object.defineProperty(this, 'loadFrame', {value: function (systemId, version, ideLogName, callback) {
      ...
      (frame !== null && frame !== undefined ? frame : Kotlin.throwNPE()).onload = $fb.bind(null, version, ideLogName, callback, frame);
    ...
    System: {value: (function () {
      return Kotlin.createClass(null, null, /** @lends _.com_jetbrains_browserConnection_bootstrap.System.prototype */ {
        connect: {value: function (buildInfo, host, port, connected, disposable) {
        }}
      });
    })()}
  });
  ...
  Kotlin.p(_, 'com_jetbrains_browserConnection_bootstrap', function () {
    function $f0(requestId, it) {
      _.com_jetbrains_browserConnection_bootstrap.requestIdToOriginUrl.put(requestId, it.url);
    }
    function $f1(it) {
      if (it.tabId !== -1) {
    ...
    Object.defineProperty(this, 'addAllowAnyOrigin', {value: function (it, requestorUrl) {
      var responseHeaders = it.responseHeaders;
      var tmp$0;
      if (requestorUrl === null || requestorUrl === undefined)
        tmp$0 = "*";
      else {
        var url = Kotlin.modules['browser-ext-platform'].org_jetbrains_io.get_asParsedUrl(requestorUrl);
        if (url === null || url === undefined) {
          tmp$0 = "*";
        }
         else {
          tmp$0 = url.scheme + "://" + url.authority;
        }
      }
      var allowValue = tmp$0;
      if (responseHeaders !== null && responseHeaders !== undefined) {
        {
          var tmp$1 = Kotlin.arrayIterator(responseHeaders);
          while (tmp$1.hasNext()) {
            var header = tmp$1.next();
            if (Kotlin.equals(header.name, "Access-Control-Allow-Origin")) {
              var tmp$2;
              if (header.value === null || header.value === undefined || (!Kotlin.equals(header.value, "*") && !(((tmp$2 = header.value) !== null && tmp$2 !== undefined ? tmp$2 : Kotlin.throwNPE()).indexOf(allowValue) !== -1))) {
